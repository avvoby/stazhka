/* ============================================================
   Stazhka Search — секция SPA (S-1 ... S-9∑)
   ============================================================ */
(function(){
  const SECTION_ID = 'search';
  const DEFAULT_SCREEN = 'S-1';
  const Store = window.Store;
  const screens = window.SearchScreens;
    ...
    goTo(id){
      if(byId[id]){
        ...
        if(id === 'S-7' && !state.applyPhase){
          ...
          setTimeout(() => {
            if(state.applyPhase === 'loading' && state.screenId === 'S-7'){
              state.applyPhase = 'letter';
              render();
            }
          }, 1400);
        }
