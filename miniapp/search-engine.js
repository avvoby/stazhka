...
function renderTabbar(){
  tabbarEl.innerHTML = TABS.map(t => `
    <button class="tab ${t.id==='search'?'active':''} ${t.disabled?'disabled':''}" data-tab="${t.id}">
      ${t.icon}
      <span>${t.label}</span>
    </button>
  `).join('');
...
function render(){
  const screen = byId[state.screenId] || byId['S-1'];
...
function applyTweaks(){
  ...
  const emBtn = document.getElementById('tw-empty');
  if(emBtn){ emBtn.classList.toggle('on', t.emptyPicks); emBtn.textContent = t.emptyPicks?'вкл':'выкл'; }
}
...
document.getElementById('tw-empty').addEventListener('click', ()=> setTweak('emptyPicks', !window.__TWEAKS.emptyPicks));
